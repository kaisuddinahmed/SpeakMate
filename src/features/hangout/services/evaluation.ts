...
export async function evaluateConversation(transcript: TranscriptItem[]): Promise<EvaluationResult> {
    ...
    const conversationText = transcript
        .map((msg) => `${msg.speaker === 'user' ? 'Student' : 'SpeakMate'}: ${msg.text}`)
        .join('\n');

    // Calculate Audio Confidence Score (Hybrid Approach)
    const userMessages = transcript.filter(m => m.speaker === 'user' && m.confidence !== undefined);
    let calculatedPronunciationScore: number | null = null;

    if (userMessages.length > 0) {
        const totalConf = userMessages.reduce((sum, m) => sum + (m.confidence || 0), 0);
        const avgConf = totalConf / userMessages.length;
        // Map 0-1 to 1.0-9.0 Band. Deepgram is usually high, so we scale strictly.
        // OLD: (avgConf * 10) - 1
        // NEW (Strict): 1.0 -> 9.0, 0.95 -> 8.0, 0.9 -> 7.0
        // Round to nearest 0.5
        let rawScore = (avgConf * 20) - 11;
        rawScore = Math.max(1.0, Math.min(9.0, rawScore));
        calculatedPronunciationScore = Math.round(rawScore * 2) / 2;
        console.log(`[Evaluation] Hybrid Pronunciation: AvgConf=${avgConf.toFixed(3)} -> Score=${calculatedPronunciationScore.toFixed(1)}`);
    }

    // 3. Define Rubric (IELTS Aligned)
    const conversationRubric = `
You are an Expert English Speaking Performance Evaluator. Evaluate ONLY the STUDENT's performance.
...
=== EVALUATION CRITERIA (Band 0-9, 0.5 increments - Standard CEFR Alignment) ===
...
4. PRONUNCIATION (Estimated via Intelligibility)
9: Highly intelligible text; natural phrasing implies good rhythm.
8: Clear and easy to follow.
7: Mostly clear; minor phrasing issues.
6: Understandable; some awkwardness in phrasing.
5: Understandable but requires effort to parse meaning.
4: Hard to follow due to poor phrasing/structure.
3: Many unintelligible parts.
2: Mostly unintelligible.
1: Speech largely unintelligible.

IMPORTANT: 
- We have calculated a Pronunciation Score of ${calculatedPronunciationScore ? calculatedPronunciationScore.toFixed(1) : "N/A"} based on audio signal analysis.
- If the score is provided above (not N/A), USE IT as the base for the Pronunciation score.
- For Feedback: Evaluate phrasing, rhythm, and flow based on the text (Intelligibility).
...
=== COHERENCE & COHESION INSTRUCTION ===
- Analyze "Fluency & Coherence" by checking if the Student's answers logically connect to the AI's questions.
- If the student goes off-topic or gives disjointed answers, reduce the "Fluency" score.
- Look for cohesive devices (however, therefore, because) to link ideas.
...
- "recommendedPractice": Provide ONE actionable exercise targeting the CURRENT SESSION'S specific errors.
  - DO NOT give generic advice like "watch movies" or "listen to podcasts".
  - MUST reference specific words/grammar from the transcript (e.g. "Practice past tense verbs like 'buyed' -> 'bought'").
...
- "explanation": Brief reason (e.g. "Subject-Verb Agreement", "Topic Coherence")
...
`;
    ...
    try {
        ...
        // Override with Calculated Score if available (Hybrid Model)
        let pronunciationScore = roundToHalf(parsed.pronunciation || 0);
        if (calculatedPronunciationScore !== null) {
            pronunciationScore = roundToHalf(calculatedPronunciationScore);
        }
