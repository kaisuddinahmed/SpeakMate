...
function generateImprovementSuggestions(
  ...
  if (scores.fluency < 6) {
    if (metrics.hesitationMarkers > metrics.totalWords * 0.05) {
      ...
      suggestions.push(
        `Reduce hesitation markers (um, uh, er) - you used ${metrics.hesitationMarkers} in ${metrics.totalWords} words (${rate}%)`
      );
    }
    if (metrics.avgSentenceLength < 8) {
      suggestions.push(
        `Try to extend your sentences - your average was ${metrics.avgSentenceLength.toFixed(1)} words per sentence`
      );
    }
  ...
  if (scores.vocabulary < 6) {
    if (metrics.lexicalDiversity < 0.5) {
      ...
      suggestions.push(
        `Expand vocabulary variety - only ${percentage}% of your words were unique (${metrics.uniqueWords} unique out of ${metrics.totalWords})`
      );
    }
  }
  ...
  if (scores.grammar < 6 || scores.fluency < 6) {
    if (metrics.discourseMarkers === 0) {
      suggestions.push(
        `Use linking words like 'however', 'moreover', 'therefore' to connect your ideas more clearly`
      );
    }
    if (metrics.complexSentences < 2 && metrics.totalWords > 50) {
      suggestions.push(
        `Try using more complex sentences with 'because', 'although', 'while' to show range`
      );
    }
  }
...
/**
 * Evaluate a single question-answer pair (IELTS style)
 */
export async function evaluateAnswer({
  ...
  question: string;
  answer: string;
}): Promise<QuestionAnswerEvaluation> {
  const userContent = `
Question:
${question}
...
`;
  ...
  let parsed: QuestionAnswerEvaluation;
  try {
...
/**
 * Evaluate a full conversation (Hangout session)
 */
    ...
    metrics: parsed.metrics || {
      ...
      hesitationRate:
        metrics.totalWords > 0
          ? parseFloat(((metrics.hesitationMarkers / metrics.totalWords) * 100).toFixed(1))
          : 0,
      discourseMarkersUsed: metrics.discourseMarkers,
